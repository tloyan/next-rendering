...
La deuxième stratégie de rendu coté serveur des RSC est le rendu dynamique. Cobntrairement au rendu statique, le rendu dynamique régénère une nouvelle page sur chaque requête.
...
📑 Le liens vers la doc [https://nextjs.org/docs/app/building-your-application/rendering/server-components#dynamic-rendering](https://nextjs.org/docs/app/building-your-application/rendering/server-components#dynamic-rendering)
...
Les segments de routes dynamiques comme la notre : [`exercises/dynamic-rendering/[id]`](exercises/dynamic-rendering/1) vont créer automatiquement des rendus dynamiques.
...
_ps : pense à cause où le post n’est pas trouvé_
...
Les appels aux sources de données et le rendu coté serveur peut parfois être long. Pour simuler cela dans notre page nous allons utiliser le bout de code suivant :
...
- 🐶Récupère la liste des post avec `getPosts` de `'@/db/sgbd’` et retourne tous les ids.

<aside>
💡 Constate avec la commande `build` que les routes `/exercises/dynamic-rendering/x` sont pré-générés.
...
👨‍✈️ Hugo le chef de projet nous indique que les `posts` peuvent être mis à jour de temps en temps par les rédacteurs. Mais dans le cas actuel, une fois les pages générées, elle ne sont plus mises à jour en cas de changement de données en BDD. (fais le test en changeant le `title` dans `db.json`)

Ajoute le bout de code qui permet un revalidation des données après X secondes.
...
👨‍✈️ Hugo le chef de projet de demande de faire une autre route dans laquelle nous allons pouvoir filtrer des posts en query params par exemple avec `filter` et `text`[?filter=title&text=superpost](http://localhost:3000/exercises/dynamic-rendering/search-params?filter=title&text=a)

- `filter :`le champs a filter
...
- **🐶 Le composant :** `exercise/dynamic-rendering/search-params/page.tsx` est statique par default. (constate le avec un `build/start` de production)
...
Remplir le formulaire le [formulaire de FeedBack](https://go.mikecodeur.com/cours-next-avis?entry.1912869708=Next%20PRO&entry.1430994900=Rendering%20Avancée&entry.533578441=02%20dynamic-rendering-function).
