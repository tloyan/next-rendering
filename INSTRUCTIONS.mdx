...
C’est la stratégie par défaut de `Next`. Cette stratégie consiste à générer le rendu de chaque route au `“build time”` c’est à dire lors [de la compilation](https://nextjs.org/docs/architecture/nextjs-compiler) ou après `une demande de revalidation` .

En tant que développeur, il vaut mieux avoir conscience que le comportement de notre application peut être impacté selon que nous soyons en mode développement (`'pnpm dev'`) ou en mode production (’`pnpm build`’ / ‘`pnpm start`’).
...
🐶 Dans cet exercice, tu vas devoir afficher un segment de route statique. Cela est le comportement par défaut d’un `React Server Component` avec `Next`.

Dans la route [/exercises/static-rendering](http://localhost:3000/exercises/static-rendering) il va falloir faire un appel à une source de données.
...
Et afficher ces `posts` statiquement sur cette route. Pour vérifier que le comportement est correct, tu peux modifier la source de données en ajoutant ou modifiant des `Posts` par exemple. Comme nous sommes en statique, le contenu ne devrait plus être modifié à l’écran.
...
Ce mode de fonctionnement ressemble à un système de cache (infini) car on accède une seule fois à la source de données. Ces données peuvent ensuite être dispatchées à plusieurs serveurs `CDN`.

Fichiers

- `exercise/static-rendering/page.exercice.tsx`

## Bonus

### 1. 🚀 Revalidation

Les routes statiques peuvent être regénérées [en se basant sur le temps](https://nextjs.org/docs/app/building-your-application/data-fetching/fetching-caching-and-revalidating#revalidating-data)

```tsx
export const revalidate = 3600 // revalidate at most every hour
```

ou [manuellement](https://www.notion.so/1-Static-Rendering-b518ffb87d1241d9b7faa9984b54d1f9?pvs=21)
...
Remplir le formulaire le [formulaire de FeedBack](https://go.mikecodeur.com/cours-next-avis?entry.1912869708=Next%20PRO&entry.1430994900=04%20Rendering%20Avancée&entry.533578441=01%20Static%20Rendering).
