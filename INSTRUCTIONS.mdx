...
Nous avons toujours besoin de `React Client Component` (`RCC`) pour :
...
📑 Le lien vers la doc [https://nextjs.org/docs/app/building-your-application/rendering/client-components](https://nextjs.org/docs/app/building-your-application/rendering/client-components)

## Exercice

Comme nous l’avons vu, ajouter du code client sans la directive `‘use client’` génère une erreur. Ici nous avons un `RSC` (nous ne pouvons donc pas utiliser la directive `‘use client’`) mais nous voulons inclure des composants enfants clients
...
Nous voulons utiliser un `RCC`, un simple composant `<Counter />`
...
Nous utilisons souvent des librairies `React` externes dans nos projets. Par exemple ici nous avons une librairie externe.
...
- 🐶 Dans un premier temps, essaie d’utiliser cette libraire en dessous du compteur dans `page.tsx`
...
Nous avons aussi un `RCC` `<RccFetch>` qui appelle une fonction `getData` :
...
Un `fetch` qui contient des données sensibles comme un `token` :
...
Ce module va déclencher une erreur en cas d’import de code serveur dans le client
...
Remplir le formulaire le [formulaire de FeedBack](https://go.mikecodeur.com/cours-next-avis?entry.1912869708=Next%20PRO&entry.1430994900=04%20Rendering%20Avancée&entry.533578441=06%20rcc-external-lib).
